...
import type { CommentDocument, DiscussionDocument, UserDocument } from './types';

const DEMO_TEAM_ID = 'demo-team-acme';
...
const demoDiscussions = [
  {
    _id: 'demo-discussion-onboarding',
  ...
  {
    _id: 'demo-discussion-design-review',
  ...
  {
    _id: 'demo-discussion-api-versioning',
  ...
  {
    _id: 'demo-discussion-release-checklist',
  ...
  {
    _id: 'demo-discussion-retrospective',
...
const isDemoSeedPresent = async (): Promise<boolean> => {
  const existingTeam = await getTeamsCollection().findOne({ _id: DEMO_TEAM_ID });
  return existingTeam !== null;
};

const buildDiscussion = (
  discussion: (typeof demoDiscussions)[number],
  usersById: Map<string, UserDocument>,
): DiscussionDocument => {
  const author = usersById.get(discussion.authorId);

  if (!author) {
    throw new Error(`Missing author for discussion ${discussion._id}`);
  }
...
const buildComment = (
  comment: (typeof demoComments)[number],
  usersById: Map<string, UserDocument>,
): CommentDocument => {
  const author = usersById.get(comment.authorId);

  if (!author) {
    throw new Error(`Missing author for comment ${comment._id}`);
  }
...
export const seedDemoData = async (): Promise<void> => {
  ...
  const users: UserDocument[] = demoUsers.map((user) => ({
    ...user,
  ...
  for (const user of users) {
    await getUsersCollection().updateOne({ _id: user._id }, { $set: user }, { upsert: true });
  }

  for (const discussion of demoDiscussions) {
    const document = buildDiscussion(discussion, usersById);
    await getDiscussionsCollection().updateOne(
      { _id: document._id },
      { $set: document },
      { upsert: true },
    );
  ...
  for (const comment of demoComments) {
    const document = buildComment(comment, usersById);
    await getCommentsCollection().updateOne(
      { _id: document._id },
      { $set: document },
      { upsert: true },
    );
...
export const createTeam = async (name: string): Promise<string> => {
  const teamId = nanoid();
  await getTeamsCollection().insertOne({
    _id: teamId,
    name,
    description: '',
    createdAt: Date.now(),
